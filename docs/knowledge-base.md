...
> **服务方法载荷目前跨语言不一致**（实测）
>
> C++ 与 C# 的方法载荷**带** FieldMask 段（与结构体同构）；Go 与 Python 的**不带**。
> 两端各自自洽，但彼此的字节流对不上 —— 见 §10 与 §12。
...
| 组合 | 基本类型 | 非 64 位整数 | bool | 嵌套 struct | 服务方法载荷 |
|---|---|---|---|---|---|
| **C++ ↔ C#** | 一致 | 一致 | 一致 | 一致 | 一致 |
| **C++ ↔ Python** | 一致 | 一致 | 一致 | **掩码错位** | **互不兼容** |
| **C++ ↔ Go** | 一致 | 一致 | 一致 | 未实测 | **互不兼容** |
...
**服务方法载荷**：Go 与 Python 的方法载荷**不含 FieldMask**，C++ 与 C# 的**含**。
因此服务调用目前无法跨语言 —— 这一条独立于本次修复，属既有分歧。
...
> **本次已修复**：Go 整数宽度、Go 生成代码无法编译、C# 命名空间断裂、
> 以及 `(skipcomp)` 造成的跨语言不兼容（该标记已从语法中移除）。
> 修复后四种语言在基本类型上输出逐字节一致，由 §11 的黄金向量用例守护。
...
Go 与 Python 生成的**方法载荷不含 FieldMask**，C++ 与 C# 的**含**（`[fmLen][fmask][参数]`）。
因此服务调用目前无法跨语言。

这不是本次改动引入的：两端各自自洽，但彼此的字节流对不上。已在 §10 标注。
